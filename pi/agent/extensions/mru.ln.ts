...
async function pickMruFile(ctx: ExtensionContext) {
	if (!ctx.hasUI) {
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("mru", {
		...
		handler: async (_args, ctx) => { await pickMruFile(ctx) },
	})

	pi.registerShortcut("ctrl+;", {
		description: "Pick a recently used file (MRU)",
		handler: async (ctx) => { await pickMruFile(ctx) },
	})
}
