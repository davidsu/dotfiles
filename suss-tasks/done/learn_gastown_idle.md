...
|-----------|-----|-----------|
| **`gt mail`** | async, cross-agent, survives crashes | routed by address (`<rig>/witness`, `<rig>/refinery`, `mayor/`); persisted as **Beads (Dolt) ledger entries** — observable + audit trail; lifecycle read→process→delete. Router in [internal/mail/router.go](/internal/mail/router.go) |
...
- **Beads ledger** (Dolt) — molecule/step progress
- **git** — branch + worktree (the persistent "sandbox")
- **the hook** — `hook_bead` pins the work to the agent identity

**Wake = supervisor respawns a fresh session.** A **daemon** (3-min heartbeat tick) + **witness** (per-rig patrol) detect a dead session that still has pinned work and call `SessionManager.Start()` → new tmux session in the existing worktree → `SessionStart` hook fires `gt prime --hook` → the new session discovers its next step via `bd mol current`.
...
Tradeoff: respawn needs a real persistent store (Beads/Dolt) + a running supervisor daemon + tmux/process control — heavier than our file-channel + hooks, but crash-proof and genuinely human-out-of-the-loop at scale (20-30 agents).
