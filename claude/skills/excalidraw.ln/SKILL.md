...
> @ `8646fcc9f74f38539c6cdb4c969723336a96ddcd` (2026-08-19). To diff against upstream, clone the repo at that SHA.
> Local modifications:
> 1. `name:` renamed `excalidraw-diagram` → `excalidraw` (this repo's skill slug).
> 2. Render commands point at `~/.claude/skills/excalidraw/references` (our symlink target).
> 3. `references/render_template.html`: import pinned to `@excalidraw/excalidraw@0.18.0` (unpinned esm.sh 404s on a transitive dep).
> 4. `references/color-palette.md`: replaced with our house style (hand-drawn, white canvas, classic excalidraw palette).
