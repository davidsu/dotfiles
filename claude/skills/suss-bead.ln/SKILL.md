...
- **Exporting beads** (`/suss-bead export <id>`): Read [EXPORT.md](resources/EXPORT.md) first. Follow its algorithm exactly.
...
| [EXPORT.md](resources/EXPORT.md)         | `/suss-bead export <bead-id>` — export to markdown     |
...
~/.claude/skills/suss-bead/scripts/bd-section <bead-id> [section-slug]

# Explore a full bead tree with summaries only
~/.claude/skills/suss-bead/scripts/bd-explore <bead-id>
...
~/.claude/skills/suss-bead/scripts/bd-validate-mermaid --string 'graph LR; A --> B'
```

## Invocations

### `/suss-bead explore <bead-id>`
...
```bash
~/.claude/skills/suss-bead/scripts/bd-explore $1
```
